...
public class RogueLike {
    ...
    public static void playerNextMoveAction(int lastPosX, int lastPosY, int nextPosX, int nextPosY) {
            if (!(invisibleRoom[nextPosY][nextPosX] == enemy)){
            ...
            else if (invisibleRoom[nextPosY][nextPosX] == enemy && enemyHP > 0) {
                room[playerPosY][playerPosX] = enemy;
                ...
                if (enemyHP <= 0){
                    ...
                    countEnemies--;
                    enemyHP = enemyHPLast;
                    System.out.println("You has killed the enemy! Players HP= " + playerHP);
                } else {
                    room[playerPosY][playerPosX] = enemy;
                    room[lastPosY][lastPosX] = player;
                    playerPosX = lastPosX;
                    playerPosY = lastPosY;
                    System.out.println("ALERT! Enemy give damage " + enemyAttack + ". Player HP now is " + playerHP + " Enemy HP now is " + enemyHP);
                }
            } else {
                room[playerPosY][playerPosX] = player;
                room[lastPosY][lastPosX] = readyCell;
